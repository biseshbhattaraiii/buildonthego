...
async function runOAuthFlow() {
  ...
  const isHeadless = !process.env.DISPLAY && process.platform !== 'darwin' && process.platform !== 'win32';

  if (isHeadless) {
    // VM / headless mode — print URL and ask for code manually
    console.log('\n' + chalk.bold('Headless environment detected.'));
    console.log(chalk.cyan('Open this URL on any browser and paste the code back here:\n'));
    ...
    const { tokens } = await oauth2Client.getToken(manualCode.trim());
    oauth2Client.setCredentials(tokens);
    const config = getConfig();
    config.googleRefreshToken = tokens.refresh_token;
    saveConfig(config);
    console.log(chalk.green('Google authentication successful.\n'));
    return oauth2Client;
  }
